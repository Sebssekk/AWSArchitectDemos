...
aws ssm get-parameter --name /ec2/keypair/$winKeyId --with-decryption --query Parameter.Value --output text > demo-win-key.pem
aws ssm get-parameter --name /ec2/keypair/$linKeyId --with-decryption --query Parameter.Value --output text > demo-lin-key.pem